...
export function useBehaviorSubject(init, autoUnsubscribe = true) {
    const proxyData = observable(init, () => stream$.next(proxyData));
    const [stream$, sstream] = useState(new BehaviorSubject(proxyData));
    if (autoUnsubscribe) {
        useEffect(() => {
            return () => stream$.unsubscribe();
        }, []);
    }
    return [stream$, sstream];
}
